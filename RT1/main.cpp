...
void create_scene()
{
  ...
  auto plane1 = std::make_shared<Plane>(float3(+0.0f, -1.0f, +0.0f), float3(0.0f, 1.0f, 0.0f), new Diffuse(float3(0.8f, 0.4f, 0.3f)));
  myScene.push_back(plane1);
  auto sphere2 = std::make_shared<Sphere>(float3(-8.0f, 1.0f, +10.0f),2.0, new  IdealMirror(float3(0.20f, 0.70f, 0.50f)));
  auto sphere3 = std::make_shared<Sphere>(float3(-5.0f, 6.0f, +1.0f), 1.0, new Diffuse(float3(0.10f, 0.80f, 0.30f)));
  auto triangle1 = std::make_shared<Triangle>(float3(15.0f, 3.0f, 1.0f), float3(10.0f, 5.0f, 5.0f), float3(5.0f, 2.0f, 5.0f), new Diffuse(float3(0.50f, 0.30f, 0.00f)));
  auto square1 = std::make_shared<Square>(float3(-7.0f, 4.0f, -6.0f), float(5.0f), new Diffuse(float3(0.1f, 0.2f, 0.3f)));
  auto parallelepiped1 = std::make_shared<Parallelepiped>(float3(2.0f, 2.0f, 5.0f), float3(5.0f, 8.0f, 0.0f), new Diffuse(float3(0.5f, 0.5f, 0.05)));
  myScene.push_back(triangle1);
  myScene.push_back(square1);
  myScene.push_back(parallelepiped1);
